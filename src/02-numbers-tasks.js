...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const divisor = ((Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2)));
...
function roundToPowerOfTen(num, pow) {
  const reversedNumArr = num.toString().split('').reverse();
  const resultArray = [];
  reversedNumArr.forEach((item, i) => {
    if (i < pow) {
      if (+item >= 5) {
        reversedNumArr[i + 1] = +reversedNumArr[i + 1] + 1;
      }
      resultArray.push(0);
    } else {
